...
class TradingAgentsGraph:
    ...
    def __init__(
        ...
        if normalized_quick_provider and normalized_deep_provider and normalized_quick_provider != normalized_deep_provider:
            ...
            # Custom OpenAI-compatible providers (such as ZenMux) keep their
            # endpoint and key in the database-backed analysis configuration.
            # Falling back to the environment is only needed for legacy setups.
            custom_api_key = (
            ...
            if not custom_api_key:
                raise ValueError("使用自定义 OpenAI 兼容端点需要在厂家配置中设置 API Key 或设置 CUSTOM_OPENAI_API_KEY")
            ...
            if not custom_base_url:
                raise ValueError("使用自定义 OpenAI 兼容端点需要在厂家配置中设置 API 基础 URL")
