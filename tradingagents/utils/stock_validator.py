...
class StockDataPreparer:
    ...
    async def _prepare_china_stock_data_async(self, stock_code: str, period_days: int,
        ...
        try:
            ...
            from tradingagents.dataflows.interface import get_china_stock_info_unified
            # The legacy unified data APIs synchronously drive their own event loop.
            # Run them in a worker thread so this async validation path does not try
            # to nest an event loop (which otherwise makes AKShare appear unavailable).
            stock_info = await asyncio.to_thread(get_china_stock_info_unified, stock_code)
            ...
            for data_source in priority_order:
                try:
                    ...
                    if data_source == "baostock":
                        hist_result = await service.sync_single_historical_data(
                    ...
                    try:
                        ...
                        if data_source in {"tushare", "baostock"}:
